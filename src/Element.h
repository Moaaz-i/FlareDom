...
public:
    ...
    static Element create(const __FlashStringHelper* tag) {
        return Element(tag);
    }
    ...
    Element &syncLocalStorage(const String &key) {
        ...
        String js = "(function() {"
                    ...
                    "  if (saved !== null) {"
                    "    if (el.tagName === 'INPUT' || el.tagName === 'TEXTAREA' || el.tagName === 'SELECT') el.value = saved;"
                    "    else el.innerText = saved;"
                    "  }"
                    "  const save = () => {"
                    "    const val = (el.tagName === 'INPUT' || el.tagName === 'TEXTAREA' || el.tagName === 'SELECT') ? el.value : el.innerText;"
    ...
    Element &onClickSendWs(const String &wsName, const String &cmd, const String &payload = "") {
        ensureId("ws-send");
    ...
    Element &ajaxSubmit(const String &endpoint, const String &onSuccessJS = "") {
        ...
        return onEvent("submit", 
            ...
            ".then(r => r.text())"
            ".then(res => {"
            "  if(" + String(onSuccessJS.length() ? "true" : "false") + ") {"
                 + onSuccessJS +
            "  }"
