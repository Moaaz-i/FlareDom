...
public:
  ...
  Router(AsyncWebServer *srv) : server(srv) {
    ...
    // Register default catch-all not-found handler to resolve dynamic bindings
    // & auth
    server->onNotFound([this](AsyncWebServerRequest *request) {
      ...
      if (dynRoutes.count(path)) {
        if (isRouteProtected(path)) {
          if (!request->authenticate(authUsername.c_str(),
                                     authPassword.c_str())) {
            return request->requestAuthentication();
          }
        }
        AsyncWebServerResponse *response =
            request->beginResponse(200, "text/plain", dynRoutes[path]());
  ...
  void protectRoute(const String &path) { protectedRoutes.push_back(path); }
  ...
  void addRedirect(const String &from, const String &to) {
    server->on(from.c_str(), HTTP_GET,
               [to](AsyncWebServerRequest *request) { request->redirect(to); });
  }

#if defined(ESP8266) || defined(ESP32)
  void serveStatic(const String &uri, fs::FS &fs, const String &path,
  ...
  void onWsCommand(const String &command,
