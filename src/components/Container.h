...
public:
  ...
  Container &gap(const String &g) {
    gapVal = g;
    return *this;
  }
  Container &justify(const String &j) {
    justifyVal = j;
    return *this;
  }
  Container &align(const String &a) {
    alignVal = a;
    return *this;
  }
  Container &wrap(bool w) {
    wrapEnabled = w;
    return *this;
  }
  Container &maxWidth(const String &mw) {
    maxWidthVal = mw;
    return *this;
  }
  Container &padding(const String &p) {
    paddingVal = p;
    return *this;
  }
  ...
  Element render() override {
    ...
    container.style.display(layoutType).gap(gapVal).padding(paddingVal);

    if (justifyVal.length())
      container.style.justifyContent(justifyVal);
    if (alignVal.length())
      container.style.alignItems(alignVal);
    ...
    if (layoutType == "flex") {
      if (wrapEnabled)
        container.style.add("flex-wrap:wrap");
