...
struct Style {
    ...
    Style &flexDirection(const String &val) { return add("flex-direction:" + val); }
    Style &justifyContent(const String &val) { return add("justify-content:" + val); }
    Style &alignItems(const String &val) { return add("align-items:" + val); }
    Style &gridTemplateColumns(const String &val) { return add("grid-template-columns:" + val); }
};
